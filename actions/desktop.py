...
def _get_desktop() -> Path:
    # Delegates to actions/_paths.py, which reads the real (possibly
    # OneDrive-redirected) Desktop path from the Windows Shell instead of
    # assuming Path.home() / "Desktop" is always correct.
...
def _show_desktop() -> str:
    """Minimize/restore all windows (Windows: Win+D, macOS: F11/Mission
    Control shortcut, Linux: Super+D on most DEs). This never opens a
    folder and never touches any file — it only toggles window visibility."""
    if not _PYAUTOGUI:
        return "pyautogui is not installed — cannot send the Show Desktop shortcut."
    try:
        if _OS == "Windows":
...
def _open_desktop_folder() -> str:
    """Open the real Desktop *folder* (OneDrive-aware) in the system file
    manager. Distinct from _show_desktop() above."""
    desktop = _get_desktop()
    try:
        if _OS == "Windows":
        ...
        return f"Could not open the Desktop folder: {e}"
...
def desktop_control(
    ...
    parameters:
        action : wallpaper | wallpaper_url | current_wallpaper |
                 ...
                 show_desktop (Win+D, no file/folder access) |
                 open_desktop (opens the Desktop folder) |
                 task (AI-powered)

        NOTE: "move a file to the Desktop" is intentionally NOT handled
        here — that goes through file_controller's move/copy actions
        with destination="desktop", which already resolves the real
        (OneDrive-aware) Desktop path via actions/_paths.py.
    ...
    try:
        ...
        elif action in ("show_desktop", "show"):
            # Windows: Win+D toggles "minimize everything" — this is the
            # Windows *virtual desktop* action, distinct from opening the
            # Desktop *folder* (below) and distinct from moving a file
            # onto the Desktop (that's file_controller's job).
            return _show_desktop()

        elif action in ("open_desktop",):
            # Explicitly open the Desktop folder in the file manager.
            # Never triggered implicitly — only when the user actually
            # asks to see the folder's contents in Explorer/Finder.
            return _open_desktop_folder()

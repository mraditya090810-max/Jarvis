...
class _SessionRegistry:
    ...
    def close_one(self, browser_name: str) -> str:
        ...
        # Most "open chrome" requests launch the browser natively (not via
        # this automation registry), so there is often no session object to
        # close here even though the browser is genuinely open. Fall back to
        # closing the real OS process for it.
        from actions.process_utils import close_app_by_name
        result = close_app_by_name(browser_name)

        if sess and not result.get("found"):
            return f"{browser_name} closed."
        if not result.get("found"):
            return f"{browser_name} does not appear to be open."
        if result.get("still_running", 0) > 0:
            return f"Tried to close {browser_name}, but it did not fully quit — it may have a pending save/dialog."
        return f"{browser_name} closed."
    ...
    def close_all(self) -> str:
        ...
        # Also close any natively-opened browser windows (Chrome/Brave/etc.)
        # that never went through the automation registry at all.
        from actions.process_utils import close_app_by_name
        native_closed = []
        for browser_name in set(_ALIASES.values()) | {"chrome", "firefox", "brave", "edge", "opera"}:
            r = close_app_by_name(browser_name)
            if r.get("found") and r.get("still_running", 0) == 0:
                native_closed.append(browser_name)

        all_closed = sorted(set(names) | set(native_closed))
        return "All browsers closed: " + (", ".join(all_closed) if all_closed else "none")

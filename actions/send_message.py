...
def _wait_for_window(name_hint: str, timeout: float = 15.0, poll: float = 0.3) -> bool:
    """Poll for a visible window whose title contains name_hint, instead of
    guessing a fixed delay for how long an app takes to finish loading.

    WhatsApp Desktop especially can take anywhere from under a second to
    well over ten, depending on how much chat history it's syncing and how
    loaded the machine is. A fixed sleep either wastes time when the app
    loads fast, or — the far worse case — is too short, so every keystroke
    that follows (searching the contact, typing the message) goes into a
    window that isn't ready yet and lands nowhere. That silent failure is
    exactly the "sometimes nothing happens, I have to ask again" symptom.

    Falls back to a short fixed sleep and returns True if window detection
    isn't available on this platform/build, so behavior never gets worse
    than the old fixed-sleep approach — only better when it can be.
    """
    try:
    ...
    while time.time() < deadline:
        try:
            for w in gw.getAllWindows():
                if hint in (w.title or "").lower() and w.visible and w.width > 100:
                    found = w
                    break
        except Exception:
            pass
    ...
    try:
        ...
        pass
    time.sleep(0.4)  # give the activation a moment to actually take effect
    return True
...
def _open_app(app_name: str) -> bool:
    ...
    try:
        if os_name == "windows":
            ...
            return _wait_for_window(app_name, timeout=15.0)
...
def _desktop_send(app_name: str, receiver: str, message: str) -> str:
    if not _open_app(app_name):
        return (
            f"{app_name} didn't finish loading in time, so I stopped before typing "
            f"anything — nothing was sent to {receiver}. {app_name} should be open "
            "now, though, so trying again right away should be quick."
        )

    time.sleep(0.4)  # window is focused, but its own UI can still be a beat behind

...
def _ddg_news_india(query: str, max_results: int = 8) -> list[dict]:
    """Same as _ddg_news but restricted to the India news region."""
    try:
        from ddgs import DDGS
    except ImportError:
        from duckduckgo_search import DDGS
    ...
    try:
        ...
        print(f"[WebSearch] ⚠️ DDG India news() failed ({e}) — falling back to text search")
        results = _ddg_search(query, max_results=max_results)
    return results


def _startup_news_india() -> str:
    """
    Startup-briefing news: India only, never US/UK/other-country headlines
    unless the user explicitly asks for those later. Same parallel
    Gemini + DDG race as _news(), just scoped to Indian sources.
    """
    import threading

    gemini_query = (
        "Top news headlines in India today. Only include news from Indian "
        "sources about India. Do not include US, UK, or other international "
        "headlines unless they are top India-related stories."
    )
    ...
    def _try_gemini():
        try:
            ...
            print(f"[WebSearch] ⚠️ Gemini India news failed ({e})")
            _store("")

    def _try_ddg():
        try:
            results = _ddg_news_india(ddg_query, max_results=8)
            _store(_format_news(ddg_query, results))
        except Exception as e:
            print(f"[WebSearch] ⚠️ DDG India news failed ({e})")
            _store("")
    ...
    done_evt.wait(timeout=10.0)
    return result_box[0] or "No India news found."

...
# All AI calls go through the AI Router (free OpenRouter models — see
# core/ai/) instead of a direct, hardcoded Gemini client.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from core.ai import call_llm_text, call_llm_vision  # noqa: E402
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    Kullanıcı hangi dilde konuşursa konuşsun, açıklama AI Router'a
    (ücretsiz OpenRouter modeli) sınıflandırtılır. API'ye ulaşılamazsa
    dile bakmayan yapısal ipuçlarına (dosya diskte var mı, kod verilmiş mi) düşülür.
    """
    ...
    if desc:
        try:
            ...
            ans = call_llm_text(prompt, timeout=20, task="code_helper_intent").strip().lower()
            ans = ans.strip("`'\". \n")
...
Code:"""
    ...
    raw_response = call_llm_text(prompt, timeout=90, task="code_helper_write")
...
Fixed code:"""
    ...
    raw_response = call_llm_text(prompt, timeout=90, task="code_helper_fix")
...
def _find_vscode_cli() -> str | None:
    """Locate the `code` CLI across platforms without assuming it's on PATH."""
...
def _open_in_vscode_terminal(path: Path, args: list) -> bool:
    """
    Best-effort visual companion to a run: open the file in VS Code and type
    the same run command into its integrated terminal, so the user can watch
    it execute there.

    Returns True if VS Code was launched and the command was typed, False
    otherwise. This is deliberately NOT the source of truth for output or
    errors — driving the terminal via keystrokes has no way to read back
    what happened on screen, so it can never be trusted to report success,
    failure, or error text. The caller always gets the real result from a
    direct subprocess run instead; this just mirrors it visually.
    """
    interp = _RUN_INTERPRETERS.get(path.suffix.lower())
    if not interp:
        return False
    ...
    try:
        subprocess.Popen(
            [code_cli, "-r", "-g", str(path)],
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        )
    except Exception as e:
        print(f"[Code] Could not launch VS Code: {e}")
        return False

    time.sleep(1.5)  # let VS Code focus and open the file first

    try:
        import pyautogui
    except Exception:
        return False  # no way to drive the integrated terminal

    quoted_interp = " ".join(f'"{a}"' if " " in a else a for a in interp)
    quoted_args = (" " + " ".join(str(a) for a in args)) if args else ""
    command = f'{quoted_interp} "{path}"{quoted_args}'

    try:
        pyautogui.hotkey("ctrl", "`")   # open/focus the integrated terminal
        time.sleep(0.8)
        pyautogui.write(command, interval=0.02)
        pyautogui.press("enter")
    except Exception as e:
        print(f"[Code] Could not drive the VS Code terminal: {e}")
        return False
...
Updated code:"""
    ...
    try:
        raw_response = call_llm_text(prompt, timeout=90, task="code_helper_edit")
        edited   = _clean_code(raw_response)
    except Exception as e:
...
Explanation:"""
    ...
    try:
        return call_llm_text(prompt, timeout=60, task="code_helper_explain").strip()
    except Exception as e:
...
def _run_action(file_path, args, timeout, player) -> str:
    ...
    # Open it visibly in VS Code's integrated terminal so the user can watch
    # it run there — but always get the actual result from a real subprocess
    # too. Typing into the VS Code terminal via keystrokes has no way to read
    # the output back, so it can't be trusted to report errors; the subprocess
    # run is what makes error reporting accurate.
    opened_in_vscode = _open_in_vscode_terminal(p, args)
    output = _run_file(p, args, timeout)

    if opened_in_vscode:
        return f"Opened {p.name} in VS Code and ran it.\n\n{output}"
    return output
...
Optimized code:"""
    ...
    try:
        raw_response = call_llm_text(prompt, timeout=90, task="code_helper_optimize")
        optimized = _clean_code(raw_response)
    except Exception as e:
...
Be specific and actionable. If you see an error message, quote it exactly."""
        ...
        analysis = call_llm_vision(
            analysis_prompt, image_base64, mime_type="image/png",
            timeout=90, task="code_helper_screen_debug",
        ).strip()

...
# All AI calls go through the AI Router (free OpenRouter models, with
# automatic discovery/ranking/fallback/caching) instead of a direct,
# hardcoded Gemini client.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from core.ai import call_llm_text, NoFreeModelAvailable  # noqa: E402
from core.paths import get_base_dir, get_special_folder  # noqa: E402


BASE_DIR         = get_base_dir()
PROJECTS_DIR     = get_special_folder("desktop") / "JarvisProjects"
MAX_FIX_ATTEMPTS = 12
...
def _is_rate_limit(error: Exception) -> bool:
    """True when every free model the router tried was rate-limited/unavailable
    (NoFreeModelAvailable), or the error text otherwise looks like a rate
    limit — kept broad so a transient provider hiccup still gets the
    wait-and-retry treatment instead of aborting the whole build."""
    if isinstance(error, NoFreeModelAvailable):
        return True
    msg = str(error).lower()
...
_PLANNER_SYSTEM = "You are a senior software architect."


# Project-type hints for structure and run commands.
_PROJECT_TYPE_HINTS = {
    "pyqt_gui": {
        "keywords": ("pyqt", "pyqt5", "pyqt6", "qt gui", "qt interface"),
        "structure": "PyQt6 app with main window, separate ui/widgets modules, requirements.txt with PyQt6.",
        "run_command": "python main.py",
        "dependencies": ["PyQt6"],
    },
    "tkinter_gui": {
        "keywords": ("tkinter", "tk gui", "tk interface"),
        "structure": "Tkinter app with main.py entry, ui in separate module if needed.",
        "run_command": "python main.py",
        "dependencies": [],
    },
    "pygame": {
        "keywords": ("pygame", "game loop", "sprite", "2d game"),
        "structure": "Pygame project with main.py game loop, assets/ folder optional.",
        "run_command": "python main.py",
        "dependencies": ["pygame"],
    },
    "fastapi_web": {
        "keywords": ("fastapi", "rest api", "api server", "uvicorn"),
        "structure": "FastAPI app in main.py with uvicorn, routes in routes/ or inline.",
        "run_command": "python -m uvicorn main:app --host 127.0.0.1 --port 8765",
        "dependencies": ["fastapi", "uvicorn"],
    },
    "flask_web": {
        "keywords": ("flask", "flask app", "flask server"),
        "structure": "Flask app factory or single main.py with templates/ if needed.",
        "run_command": "python main.py",
        "dependencies": ["flask"],
    },
    "react_web": {
        "keywords": ("react", "jsx", "tsx", "vite", "create-react-app"),
        "structure": "React + Vite: package.json, src/App.jsx, src/main.jsx, index.html.",
        "run_command": "npm run dev",
        "dependencies": [],
    },
    "node_cli": {
        "keywords": ("node cli", "npm cli", "command line tool", "javascript cli"),
        "structure": "Node CLI with package.json bin entry and index.js.",
        "run_command": "node index.js",
        "dependencies": [],
    },
    "cli": {
        "keywords": ("cli", "command line", "terminal app", "argparse", "click"),
        "structure": "Python CLI with argparse/click in main.py.",
...
def _detect_project_type(description: str, language: str) -> str:
    """Pick the best project template from description keywords."""
    low = description.lower()
    lang = language.lower()

    if lang in ("javascript", "typescript", "js", "ts"):
        if any(k in low for k in _PROJECT_TYPE_HINTS["react_web"]["keywords"]):
            return "react_web"
        return "node_cli"

    scores: dict[str, int] = {}
    for ptype, meta in _PROJECT_TYPE_HINTS.items():
        if ptype in ("react_web", "node_cli"):
            continue
    ...
    if any(w in low for w in ("gui", "window", "interface", "form", "button")):
        return "tkinter_gui"
    if any(w in low for w in ("web", "server", "http", "api")):
        return "flask_web"
    return "cli"


def _syntax_check(project_dir: Path, file_codes: dict[str, str]) -> list[str]:
    """Return list of 'path: message' for Python syntax errors before run."""
    errors: list[str] = []
    for rel_path, code in file_codes.items():
...
def _plan_project(description: str, language: str) -> dict:
    project_type = _detect_project_type(description, language)
    type_meta = _PROJECT_TYPE_HINTS.get(project_type, _PROJECT_TYPE_HINTS["cli"])
    prompt = f"""Create a minimal, complete file plan for this project.

Language: {language}
Description: {description}
Detected project type: {project_type}
Structure guidance: {type_meta["structure"]}
Suggested run command: {type_meta["run_command"]}
Suggested dependencies: {type_meta.get("dependencies", [])}
...
{{
  ...
  "run_command": "{type_meta["run_command"]}",
  "dependencies": {json.dumps(type_meta.get("dependencies", []))}
}}
...
7. Match structure to project type ({project_type}) — GUI apps need event loop, web apps need app instance.

JSON:"""

    try:
        raw_response = call_llm_text(prompt, system=_PLANNER_SYSTEM, timeout=90, task="dev_agent_plan")
...
Code for {file_path}:"""
    ...
    try:
        raw_response = call_llm_text(prompt, timeout=120, task="dev_agent_write")
...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30) -> str:
    ...
    try:
        ...
        elif parts[0].lower() == "npm":
            # npm must run via shell on Windows for .cmd resolution
            result = subprocess.run(
                run_command,
                capture_output=True, text=True,
                encoding="utf-8", errors="replace",
...
Fixed code for {fix_path}:"""
        ...
        try:
            raw_response = call_llm_text(prompt, timeout=120, task="dev_agent_fix")
            fixed = _strip_fences(raw_response)

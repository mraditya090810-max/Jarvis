...
from PyQt6.QtWidgets import (
    ...
    QSplitter, QStackedWidget, QTextEdit, QPlainTextEdit, QVBoxLayout,
    QWidget,
)
...
class LogWidget(QPlainTextEdit):
    # NOTE: was QTextEdit. QPlainTextEdit uses a layout/rendering path built for
    # continuously-growing text and gives us setMaximumBlockCount() below — an
    # O(1) rolling trim of old lines. Previously this widget kept every line
    # ever logged for the entire process lifetime with no cap, so both the
    # underlying document and every cursor-based insert (see _step) got
    # progressively more expensive the longer a session ran. That's the
    # primary cause of JARVIS "getting slower" over a long-running session —
    # it wasn't the LLM, memory, or voice pipeline, it was this widget.
    _sig = pyqtSignal(str)

    MAX_LOG_LINES = 500  # rolling window; oldest lines are auto-dropped past this

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setReadOnly(True)
        self.setMaximumBlockCount(self.MAX_LOG_LINES)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="JARVIS", user_name="",
        ...
        self._hex_input.setPlaceholderText("#6366f1   (custom hex colour)")
...
class PluginManagerOverlay(QWidget):
    """Floating overlay listing every plugin found in plugins/*.py, with an
    enable/disable checkbox per plugin. Backed by core.plugin_loader's
    PluginRegistry.list_for_ui() and memory.config_manager's
    get_plugin_enabled()/save_plugin_enabled() — MainWindow supplies both
    through the on_plugin_list / on_plugin_toggle callbacks so this file
    never has to import the backend directly."""

    _OW, _OH = 440, 480

    def __init__(self, plugins: list[dict], parent=None):
        super().__init__(parent)
        self.on_toggle = None   # callable(name, enabled) — MainWindow binds this
        self.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self.setStyleSheet(f"""
        ...
        sub = QLabel("Toggle any plugin off without deleting the file. "
                      "Disabled plugins stay loaded but Jarvis will never call them.")
        ...
        self._rows_lay.setContentsMargins(0, 0, 4, 0)
    ...
    def set_plugins(self, plugins: list[dict]):
        """(Re)populate the list. Each dict: name, description, file, valid, error, enabled."""
        ...
        for rec in plugins:
            ...
            top = QHBoxLayout()
            name_lbl = QLabel(rec.get("name") or rec.get("file", "?"))
            name_lbl.setFont(QFont("Courier New", 9, QFont.Weight.Bold))
            valid = bool(rec.get("valid"))
            name_lbl.setStyleSheet(
                f"color: {C.TEXT if valid else C.RED}; background: transparent;"
            )
            ...
            if valid:
                ...
                cb.setChecked(bool(rec.get("enabled", True)))
                ...
                bad = QLabel("BROKEN")
                bad.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
                bad.setStyleSheet(f"color: {C.RED}; background: transparent;")
                top.addWidget(bad)
            rlay.addLayout(top)

            desc = rec.get("error") if not valid else rec.get("description", "")
            if desc:
                desc_lbl = QLabel(desc)
                desc_lbl.setWordWrap(True)
                desc_lbl.setFont(QFont("Courier New", 7))
                desc_lbl.setStyleSheet(
                    f"color: {C.RED if not valid else C.TEXT_DIM}; background: transparent;"
                )
    ...
    def _on_toggled(self, name: str, checked: bool):
        if self.on_toggle:
            self.on_toggle(name, checked)


class NewsTopicsOverlay(QWidget):
    """Floating overlay for managing the news_provider plugin's tracked
    topics: add a topic to follow, remove one, see the full list. Backed by
    plugins/news_provider.py's add_topic()/remove_topic()/list_topics() —
    MainWindow supplies on_list/on_add/on_remove callbacks so this file
    never has to import the plugin directly (same pattern as
    PluginManagerOverlay)."""

    _OW, _OH = 440, 480

    def __init__(self, topics: list[dict], parent=None):
        super().__init__(parent)
        self.on_add    = None   # callable(topic: str) -> None
        self.on_remove = None   # callable(topic: str) -> None
        self.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self.setStyleSheet(f"""
            NewsTopicsOverlay {{
                background: rgba(0, 6, 10, 245);
        ...
        hdr = QLabel("📰  NEWS TOPICS")
        hdr.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        lay.addWidget(hdr)

        sub = QLabel("Jarvis only reports news for topics listed here. Add one "
                      "below, or just tell Jarvis what you want updates on.")
        sub.setWordWrap(True)
        sub.setFont(QFont("Courier New", 8))
        sub.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent;")
        lay.addWidget(sub)
        ...
        self._input.setPlaceholderText("e.g. Champions League, Tesla stock...")
    ...
    def set_topics(self, topics: list[dict]):
        """(Re)populate the list. Each dict: id, topic, added_at."""
        while self._rows_lay.count():
            item = self._rows_lay.takeAt(0)
            w = item.widget()
            if w:
        ...
        if not topics:
            empty = QLabel("Not tracking anything yet.")
            empty.setFont(QFont("Courier New", 9))
            empty.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent;")
            self._rows_lay.addWidget(empty)
            return
        ...
        for t in topics:
            ...
            rlay.setContentsMargins(10, 4, 6, 4)

            name_lbl = QLabel(t.get("topic", "?"))
            name_lbl.setFont(QFont("Courier New", 9, QFont.Weight.Bold))
            ...
            rm_btn.setFont(QFont("Courier New", 9, QFont.Weight.Bold))
            ...
            rm_btn.clicked.connect(lambda checked=False, n=topic_name: self._on_remove_clicked(n))
            rlay.addWidget(rm_btn)

            self._rows_lay.addWidget(row)
        self._rows_lay.addStretch()

    def _on_remove_clicked(self, topic: str):
        if self.on_remove:
            self.on_remove(topic)


class AudioDeviceOverlay(QWidget):
    """Floating overlay for picking which microphone and which speakers
    Jarvis uses. Backed by core.audio_devices — MainWindow supplies the
    device lists and the save callback so this file never imports
    ...
    def __init__(self, inputs: list[str], outputs: list[str],
                 current_input: str, current_output: str, parent=None):
        super().__init__(parent)
        self.on_save = None   # callable(input_name, output_name)
        self.on_refresh = None   # callable() -> (inputs, outputs, cur_in, cur_out)
        self.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self.setStyleSheet(f"""
            AudioDeviceOverlay {{
                background: rgba(0, 6, 10, 245);
        ...
        def _lbl(txt, fs=9, bold=False, color=C.TEXT_DIM):
            w = QLabel(txt)
            w.setFont(QFont("Courier New", fs, QFont.Weight.Bold if bold else QFont.Weight.Normal))
            w.setStyleSheet(f"color: {color}; background: transparent;")
            return w

        lay.addWidget(_lbl("🎙  AUDIO DEVICES", 12, True, C.PRI))
        sep = QFrame(); sep.setFrameShape(QFrame.Shape.HLine)
        sep.setStyleSheet(f"color: {C.BORDER}; margin: 2px 0;")
        lay.addWidget(sep)
        ...
        refresh_btn.setFont(QFont("Courier New", 8))
    ...
    def set_devices(self, inputs: list[str], outputs: list[str],
                     current_input: str, current_output: str):
        DEFAULT_LABEL = "System default"
        self._input_combo.clear()
        self._input_combo.addItem(DEFAULT_LABEL)
        self._input_combo.addItems(inputs)
        idx = self._input_combo.findText(current_input) if current_input else 0
        ...
        idx = self._output_combo.findText(current_output) if current_output else 0
    ...
    def _on_refresh(self):
        ...
        if not result:
            return
        inputs, outputs, cur_in, cur_out = result
        self.set_devices(inputs, outputs, cur_in, cur_out)

    def _on_save(self):
        DEFAULT_LABEL = "System default"
        in_name = self._input_combo.currentText()
        out_name = self._output_combo.currentText()
        in_name = "" if in_name == DEFAULT_LABEL else in_name
        out_name = "" if out_name == DEFAULT_LABEL else out_name
        if self.on_save:
            self.on_save(in_name, out_name)
        self.saved.emit()
        self.hide()
...
class RemoteKeyOverlay(QWidget):
    ...
    def __init__(self, url: str, key: str, auto_login_url: str = "",
        ...
        self._status_lbl = QLabel("🔴  Remote Access: Disconnected")
        ...
        stop_btn = QPushButton("STOP REMOTE ACCESS")
        ...
        stop_btn.setStyleSheet(f"""
            QPushButton {{
                ...
                border: 1px solid rgba(248,113,113,0.35); border-radius: 5px;
            }}
            QPushButton:hover {{ background: rgba(248,113,113,0.16); border: 1px solid #f87171; }}
        """)
    ...
    def set_status_query(self, fn) -> None:
        """fn() -> dict with keys: connected, device_count, camera_active — polled once/sec."""
        self._status_fn = fn
    ...
    def _refresh_status(self) -> None:
        ...
        if not st:
            return
        if st.get("connected"):
            n = st.get("device_count", 0)
            txt = f"🟢  Remote Access: Connected ({n} device{'s' if n != 1 else ''})"
            color = C.GREEN
        else:
            txt = "🔴  Remote Access: Disconnected"
            color = C.TEXT_MED
        if st.get("camera_active"):
            txt += "  ·  📷 Camera Active"
    ...
    def _do_stop(self) -> None:
        if self._on_stop:
            self._on_stop()
        # Immediately show a fresh QR — the old one and the old session are now invalid.
        self._refresh_key()
    ...
    def mark_connected(self) -> None:
        ...
        self._qr_label.setStyleSheet(
            "color: #22c55e; background: #001a0d; border-radius: 10px;"
        )
...
class MainWindow(QMainWindow):
    ...
    _cam_label_sig    = pyqtSignal(str)                  # relabel the live-feed header (camera / browser agent)
    _clipboard_sig    = pyqtSignal(str)                  # clipboard text changed (thread-safe)
    _research_sig     = pyqtSignal(str, str, str, str)   # (session_id, query, stage, message)
    _refresh_hist_sig = pyqtSignal()
    ...
    def __init__(self, face_path: str):
        ...
        self.setWindowTitle(f"{_display} — by Aditya")
        ...
        self.on_stop_remote    = None   # callable: () -> None — Stop Remote Access
        self.on_status_query   = None   # callable: () -> dict — remote-access status snapshot
        self.on_interrupt      = None   # callable: () -> None — stop JARVIS mid-speech
        self.on_plugin_list        = None   # callable: () -> list[dict] — plugin registry snapshot
        self.on_plugin_toggle      = None   # callable: (name, enabled) -> None
        self.on_news_topics_list   = None   # callable: () -> list[dict] — tracked news topics
        self.on_news_topic_add     = None   # callable: (topic: str) -> None
        self.on_news_topic_remove  = None   # callable: (topic: str) -> None
        self.on_audio_devices      = None   # callable: () -> (inputs, outputs, cur_in, cur_out)
        self.on_audio_device_save  = None   # callable: (input_name, output_name) -> None
        self._muted            = False
        self._current_file: str | None = None
        self._remote_overlay: RemoteKeyOverlay | None = None
        self._customize_overlay: CustomizeOverlay | None = None
        self._plugin_overlay: PluginManagerOverlay | None = None
        self._news_overlay: NewsTopicsOverlay | None = None
        ...
        self._cam_title = _cam_title   # saved so start_browser_stream()/relabeling can reach it later
        ...
        self._research_sig.connect(self._update_research_status)
        self._refresh_hist_sig.connect(self._refresh_research_history)
        self._reconfig_sig.connect(self._show_setup)
        self._camera_sig.connect(self._show_camera_frame)
        self._cam_stream_sig.connect(self._on_cam_stream)
        self._cam_frame_sig.connect(self._on_cam_frame)
        self._cam_label_sig.connect(self._cam_title.setText)
    ...
    # --- Live browser-agent feed in the same HUD area (no OS browser window) ---
    # Reuses the camera stack/label/frame plumbing above — a headless Playwright
    # screenshot stream is visually indistinguishable from a camera stream from
    # the widget's point of view, it's just JPEG bytes pushed into the same label.
    def start_browser_stream(self, label: str = "◈  BROWSER AGENT") -> None:
        """Thread-safe. Switch the HUD area to live-feed mode under a custom
        label (called before a browser_agent run starts pushing frames)."""
        self._cam_label_sig.emit(label)
        self._cam_stream_sig.emit(True)

    def push_browser_frame(self, img_bytes: bytes) -> None:
        """Thread-safe. Push one JPEG/PNG frame (e.g. a Playwright screenshot)
        into the live-feed area."""
        self._cam_frame_sig.emit(img_bytes)

    def stop_browser_stream(self) -> None:
        """Thread-safe. Return the HUD area to the animated face and restore
        the default 'CAMERA FEED' label."""
        self._cam_stream_sig.emit(False)
        self._cam_label_sig.emit("◈  CAMERA FEED")
    ...
    def resizeEvent(self, event):
        ...
        if self._plugin_overlay and self._plugin_overlay.isVisible():
            ow, oh = PluginManagerOverlay._OW, PluginManagerOverlay._OH
            oh = min(oh, cw.height() - 16)
            self._plugin_overlay.setGeometry(
                (cw.width()  - ow) // 2,
                (cw.height() - oh) // 2,
                ow, oh,
            )
        if self._news_overlay and self._news_overlay.isVisible():
            ow, oh = NewsTopicsOverlay._OW, NewsTopicsOverlay._OH
            oh = min(oh, cw.height() - 16)
            self._news_overlay.setGeometry(
                (cw.width()  - ow) // 2,
                (cw.height() - oh) // 2,
                ow, oh,
            )
        if self._audio_overlay and self._audio_overlay.isVisible():
            ow, oh = AudioDeviceOverlay._OW, AudioDeviceOverlay._OH
            self._audio_overlay.setGeometry(
                (cw.width()  - ow) // 2,
                (cw.height() - oh) // 2,
                ow, oh,
            )
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("by Aditya", C.PRI_DIM))
        lay.addSpacing(8)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        lay.addWidget(_sec("RESEARCH WORKSPACE"))
        self._research_status_lbl = QLabel("Idle — no active research session")
        self._research_status_lbl.setFont(QFont("Courier New", 8))
        self._research_status_lbl.setWordWrap(True)
        self._research_status_lbl.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        lay.addWidget(self._research_status_lbl)

        self._research_stage_lbl = QLabel("Status: waiting for a query")
        self._research_stage_lbl.setFont(QFont("Courier New", 7))
        self._research_stage_lbl.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent;")
        lay.addWidget(self._research_stage_lbl)
        ...
        refresh_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        refresh_btn.setFont(QFont("Courier New", 8))
        refresh_btn.setStyleSheet(f"background: {C.PANEL}; color: {C.TEXT}; border: 1px solid {C.BORDER}; border-radius: 3px;")
        refresh_btn.clicked.connect(self._do_refresh_research_history)
        btn_row.addWidget(refresh_btn)
        ...
        cancel_btn.setStyleSheet(f"background: {C.PANEL}; color: {C.RED}; border: 1px solid {C.BORDER}; border-radius: 3px;")
        cancel_btn.clicked.connect(self._do_cancel_research)
        btn_row.addWidget(cancel_btn)
        lay.addLayout(btn_row)
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        plugin_btn = QPushButton("🔌  PLUGIN MANAGER")
        ...
        news_btn = QPushButton("📰  NEWS TOPICS")
        news_btn.setFixedHeight(26)
        news_btn.setFont(QFont("Courier New", 7))
        news_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        news_btn.setStyleSheet(_BTN_STYLE_DIM)
        news_btn.clicked.connect(self._open_news_topics)
        lay.addWidget(news_btn)
    ...
    def write_log(self, text: str):
        """Thread-safe log line — usable from inside MainWindow itself, not
        just through the JarvisUI facade (which has its own copy of this)."""
        self._log_sig.emit(text)

    def show_content(self, title: str, text: str):
        """Thread-safe content-panel update — usable from inside MainWindow
        itself, not just through the JarvisUI facade (which has its own
        copy of this)."""
        self._content_sig.emit(title[:48], text[:4000])
    ...
    def _build_input_row(self) -> QHBoxLayout:
        ...
        self._input.setStyleSheet(f"""
            QLineEdit {{
                background: #050609; color: {C.WHITE};
    ...
    def _update_research_status(self, session_id: str, query: str, stage: str, message: str):
        try:
            self._research_status_lbl.setText(
                f"[{session_id[:8]}] {query[:34]} — {stage.upper()}"
            )
            self._research_stage_lbl.setText(message)
            self._log.append_log(f"RESEARCH: {stage} — {message}")
            self._show_content(f"Research: {stage}", message)
        except Exception as exc:
            traceback.print_exc()
            self._log.append_log(f"ERR: Research UI update failed — {exc}")
            try:
                self._show_content("Research UI Error", f"Research update failed: {exc}")
            except Exception:
                pass

    def _refresh_research_history(self):
        try:
            rm = get_research_manager()
            entries = rm.history(10)
            if not entries:
                self.show_content("Research History", "No past research sessions found.")
                return
            lines = [
                f"{entry['created_at']} {entry['id'][:8]} [{entry['status']}] {entry['query']}"
                for entry in entries
            ]
            self.show_content("Research History", "\n".join(lines))
        except Exception as e:
            self._log.append_log(f"ERR: Research history refresh failed — {e}")

    def _do_refresh_research_history(self):
        self._refresh_hist_sig.emit()

    def _do_cancel_research(self):
        try:
            rm = get_research_manager()
            result = rm.cancel()
            self._log.append_log(f"SYS: {result}")
            self.show_content("Research", result)
            self._research_stage_lbl.setText("Cancel requested.")
        except Exception as e:
            self._log.append_log(f"ERR: Research cancel failed — {e}")
            self.show_content("Research", f"Cancel failed: {e}")
    ...
    # ── Settings overlays are mutually exclusive ────────────────────────────────

    def _hide_settings_overlays(self):
        """Floating panels (Customise, Plugin Manager, Audio Devices, Remote)
        share the same central-widget space, so only one should ever be
        visible at a time — otherwise they stack on top of each other."""
        if self._customize_overlay and self._customize_overlay.isVisible():
            self._customize_overlay.hide()
        if self._plugin_overlay and self._plugin_overlay.isVisible():
            self._plugin_overlay.hide()
        if self._news_overlay and self._news_overlay.isVisible():
            self._news_overlay.hide()
        if self._audio_overlay and self._audio_overlay.isVisible():
    ...
    def _open_remote(self):
        ...
        auto   = result[2] if len(result) >= 3 else ""
        manual = result[3] if len(result) >= 4 else url
        self._hide_settings_overlays()
        cw  = self.centralWidget()
        ow, oh = RemoteKeyOverlay._OW, RemoteKeyOverlay._OH
        ov  = RemoteKeyOverlay(url, key, auto_login_url=auto, manual_url=manual,
                               expiry_secs=600, parent=cw)
        ov.set_new_key_callback(self.on_remote_clicked)
        ov.set_stop_callback(self.on_stop_remote)
        ov.set_status_query(self.on_status_query)
        ov.setGeometry(
            (cw.width()  - ow) // 2,
            (cw.height() - oh) // 2,
            ow, oh,
    ...
    def _open_plugin_manager(self):
        plugins = self.on_plugin_list() if self.on_plugin_list else []
        self._hide_settings_overlays()
        cw = self.centralWidget()
        ...
        ov.on_toggle = self._toggle_plugin
        ov.show()
        self._plugin_overlay = ov

    def _toggle_plugin(self, name: str, enabled: bool):
        if self.on_plugin_toggle:
            self.on_plugin_toggle(name, enabled)
        state = "enabled" if enabled else "disabled"
        self.write_log(f"SYS: Plugin '{name}' {state}.")

    # ── News Topics ───────────────────────────────────────────────────────────

    def _open_news_topics(self):
        topics = self.on_news_topics_list() if self.on_news_topics_list else []
        self._hide_settings_overlays()
        cw = self.centralWidget()
        ov = NewsTopicsOverlay(topics, parent=cw)
        ow, oh = NewsTopicsOverlay._OW, NewsTopicsOverlay._OH
        oh = min(oh, cw.height() - 16)
        ov.setGeometry(
            (cw.width()  - ow) // 2,
        ...
        ov.on_add = self._add_news_topic
        ov.on_remove = self._remove_news_topic
        ov.show()
        self._news_overlay = ov

    def _refresh_news_overlay(self):
        if self._news_overlay and self._news_overlay.isVisible() and self.on_news_topics_list:
            self._news_overlay.set_topics(self.on_news_topics_list())

    def _add_news_topic(self, topic: str):
        if self.on_news_topic_add:
            self.on_news_topic_add(topic)
        self.write_log(f"SYS: Now tracking news topic '{topic}'.")
        self._refresh_news_overlay()

    def _remove_news_topic(self, topic: str):
        if self.on_news_topic_remove:
            self.on_news_topic_remove(topic)
        self.write_log(f"SYS: Stopped tracking news topic '{topic}'.")
        self._refresh_news_overlay()

    # ── Audio Devices ─────────────────────────────────────────────────────────

    def _open_audio_devices(self):
        inputs, outputs, cur_in, cur_out = (
            self.on_audio_devices() if self.on_audio_devices else ([], [], "", "")
        )
        self._hide_settings_overlays()
        cw = self.centralWidget()
        ov = AudioDeviceOverlay(inputs, outputs, cur_in, cur_out, parent=cw)
        ow, oh = AudioDeviceOverlay._OW, AudioDeviceOverlay._OH
        ov.setGeometry(
            (cw.width()  - ow) // 2,
            (cw.height() - oh) // 2,
        ...
        ov.on_refresh = self.on_audio_devices
        ov.on_save = self._save_audio_devices
        ov.show()
        self._audio_overlay = ov

    def _save_audio_devices(self, input_name: str, output_name: str):
        if self.on_audio_device_save:
            self.on_audio_device_save(input_name, output_name)
        self.write_log("SYS: Audio device preference saved. Restart Jarvis to apply.")
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self.setWindowTitle(f"{display} — by Aditya")
...
class JarvisUI:
    ...
    def on_stop_remote(self):
        return self._win.on_stop_remote

    @on_stop_remote.setter
    def on_stop_remote(self, cb):
        self._win.on_stop_remote = cb

    @property
    def on_status_query(self):
        return self._win.on_status_query

    @on_status_query.setter
    def on_status_query(self, cb):
        self._win.on_status_query = cb
    ...
    @property
    def on_plugin_list(self):
        return self._win.on_plugin_list

    @on_plugin_list.setter
    def on_plugin_list(self, cb):
        self._win.on_plugin_list = cb
    ...
    @property
    def on_news_topics_list(self):
        return self._win.on_news_topics_list

    @on_news_topics_list.setter
    def on_news_topics_list(self, cb):
        self._win.on_news_topics_list = cb

    @property
    def on_news_topic_add(self):
        return self._win.on_news_topic_add

    @on_news_topic_add.setter
    def on_news_topic_add(self, cb):
        self._win.on_news_topic_add = cb

    @property
    def on_news_topic_remove(self):
        return self._win.on_news_topic_remove

    @on_news_topic_remove.setter
    def on_news_topic_remove(self, cb):
        self._win.on_news_topic_remove = cb
    ...
    @property
    def on_audio_device_save(self):
        return self._win.on_audio_device_save

    @on_audio_device_save.setter
    def on_audio_device_save(self, cb):
        self._win.on_audio_device_save = cb
    ...
    def report_research_progress(self, session_id: str, query: str, stage: str, message: str):
        """Thread-safe: update the research workspace status panel."""
        try:
            self._win._research_sig.emit(session_id[:16], query[:64], stage[:32], message[:2048])
        except Exception:
            traceback.print_exc()
    ...
    def start_browser_stream(self, label: str = "◈  BROWSER AGENT") -> None:
        """Thread-safe: switch the HUD area to a live browser-agent feed.
        Used by the browser_agent plugin instead of opening a real, visible
        Chrome window — the headless page is screenshotted and streamed here."""
        self._win.start_browser_stream(label)

    def push_browser_frame(self, img_bytes: bytes) -> None:
        """Thread-safe: push one screenshot frame into the live browser-agent feed."""
        self._win.push_browser_frame(img_bytes)

    def stop_browser_stream(self) -> None:
        """Thread-safe: stop the live browser-agent feed and restore the HUD face."""
        self._win.stop_browser_stream()
